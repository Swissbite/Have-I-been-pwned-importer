...
package net.daester.david.pwned.importer.by_prefix
...
class ImportByPrefix(private val status: Status, database: MongoDatabase, prefixCollectionName: String = "prefixes", private val systemProcesses: Int = Runtime.getRuntime().availableProcessors()) {
    ...
    private val maxHashOccurrenceIndex = IndexModel(BsonDocument(listOf(BsonElement("${PrefixWithHashes::maxHash.name}.${HashWithOccurrence::occurrence.name}", BsonInt32(1)))))
    private val minHashOccurrenceIndex = IndexModel(BsonDocument(listOf(BsonElement("${PrefixWithHashes::minHash.name}.${HashWithOccurrence::occurrence.name}", BsonInt32(1)))))
    private val maxCoroutineFn = systemProcesses * 20
    init {
        logger.info { "Setup import by prefix. database:${database.name}, collection:$prefixCollectionName, systemProcesses:$systemProcesses" }
    }
    ...
    private fun CoroutineScope.extractFileContent(fileChannel: ReceiveChannel<Path>): ReceiveChannel<PrefixWithHashes> =  produce(
        ...
        for (path in fileChannel) {
            ...
            val hashesAsync = async {
                extractListOfHashes(path)
            }
            val checksumAsync = async {
                calculateChecksum(path)
            }
            val hashes = hashesAsync.await()
            val checksum = checksumAsync.await()

            send(PrefixWithHashes(prefix = prefix, hashes = hashes, totalOccurrences = hashes.sumOf { it.occurrence }, minHash = hashes.minBy { it.occurrence }, maxHash = hashes.maxBy { it.occurrence }, checksum = checksum))
    ...
    private fun extractListOfHashes(path: Path): List<HashWithOccurrence> = path.toFile().readLines().map { line ->
    ...
    private suspend fun upsertInDb(dataObjects: ReceiveChannel<PrefixWithHashes>, prefixCollection: MongoCollection<PrefixWithHashes>) {
        for (dataObject in dataObjects) {
            val result = prefixCollection.withDocumentClass<JustPrefixAndChecksum>().find(eq(PrefixWithHashes::prefix.name, dataObject.prefix)).firstOrNull()
            when(result) {
                null -> {
                ...
                JustPrefixAndChecksum(dataObject.prefix, dataObject.checksum) -> status.increaseValidatedHashes(dataObject.hashes.size)
                    ...
                    val beforeReplace = prefixCollection.findOneAndReplace(eq(PrefixWithHashes::prefix.name, dataObject.prefix), dataObject, options = FindOneAndReplaceOptions().returnDocument(ReturnDocument.BEFORE))
...
data class PrefixWithHashes(val prefix: Prefix, val hashes: List<HashWithOccurrence>, val totalOccurrences: Long, val maxHash: HashWithOccurrence, val minHash: HashWithOccurrence, val checksum: String, val lastUpdated: LocalDate = LocalDate.now())
